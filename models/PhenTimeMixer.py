    ...
    def __multi_scale_process_inputs(self, x_enc, x_mark_enc):
        ...
        for i in range(self.configs.down_sampling_layers):
            ...
            if x_mark_enc_mark_ori is not None:
                # * 下采样 x_mark_enc 并确保其时间步长与 x_enc 一致
                x_mark_sampling = x_mark_enc_mark_ori[:, ::self.configs.down_sampling_window, :]
                # * 如果下采样后的 x_mark_enc 时间步长大于 x_enc，则截断
                if x_mark_sampling.shape[1] > x_enc_sampling.shape[2]:
                    x_mark_sampling = x_mark_sampling[:, :x_enc_sampling.shape[2], :]
                # * 如果下采样后的 x_mark_enc 时间步长小于 x_enc，则填充
                elif x_mark_sampling.shape[1] < x_enc_sampling.shape[2]:
                    padding_size = x_enc_sampling.shape[2] - x_mark_sampling.shape[1]
                    padding = torch.zeros((x_mark_sampling.shape[0], padding_size, x_mark_sampling.shape[2]), device=x_mark_sampling.device)
                    x_mark_sampling = torch.cat([x_mark_sampling, padding], dim=1)
